...
timeWarpFactor = 1000000; % Simulation speed factor
realStepDuration = 0.1; % Real time between sim steps in seconds, essentially framerate
...
while t < t_end && ishandle(fig)
    ...
    plotHandles = plotSolarSystem(ax, bodies, plotHandles, referenceBody);
    drawnow;

    % Fix limits
    if stepCount == 1
        axis(ax, [-scaleAU scaleAU -scaleAU scaleAU -scaleAU scaleAU]);
        ax.XLimMode = 'manual';
        ax.YLimMode = 'manual';
        ax.ZLimMode = 'manual';
    end
